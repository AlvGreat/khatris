...
# This environment returns a flat array with all observation values
...
class KhatrisEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if len(cur_piece_actions) == 1 and cur_piece_actions[0].x == self.spawn_point[0] and cur_piece_actions[0].y == self.spawn_point[1] and \
            ...
            self.garbage_sent += lock_res.garbage_sent
            self.time_alive += 1
            self.total_reward = self.garbage_sent + 0.1 * self.time_alive
