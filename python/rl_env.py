...
class KhatrisEnv(gym.Env):
    ...
    def get_action_list(self, separated=False):
        ...
        cur_piece_actions = find_moves_py(
            self.pyboard.field,
            self.pyboard.next_pieces[0],
            0,
            self.spawn_point[0],
            self.spawn_point[1],
            0,
        ...
        hold_piece_actions = find_moves_py(
            self.pyboard.field,
            self.pyboard.hold,
            0,
            self.spawn_point[0],
    ...
    def step(self, action):
        ...
        if (
            len(cur_piece_actions) == 1
            and cur_piece_actions[0].x == self.spawn_point[0]
            and cur_piece_actions[0].y == self.spawn_point[1]
            and len(hold_piece_actions) == 1
            and hold_piece_actions[0].x == self.spawn_point[0]
            and hold_piece_actions[0].y == self.spawn_point[1]
        ):
            observation = self._get_obs()
            ...
            new_board, lock_res = get_placement_res(
                self.pyboard,
                is_current_piece,
                ROTATION_DICT[action_taken.rotation_state],
                action_taken.x,
                action_taken.y,
                TSPIN_DICT[action_taken.tspin],
            )
